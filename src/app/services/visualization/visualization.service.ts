...
export class VisualizationService {
  ...
  initSvg(htmlElement, background = true, bgColor = 'white') {
    return new Promise<d3.Selection<SVGSVGElement, unknown, null, undefined>>(resolve => {
      const svg = d3Select.select(htmlElement)
        .append('svg')
        .attr('width', '100%')
        .attr('height', '100%')
  ...
  initBandXaxis(xData, width, paddingRatio = 0) {
    return new Promise(resolve => {
      const xAxis = d3Scale.scaleBand().rangeRound([0, width]).padding(paddingRatio);
      xAxis.domain(xData);
  ...
  initLinearYaxis(yData: ArrayLike<d3Array.Numeric>, height) {
    return new Promise(resolve => {
  ...
  drawLineChart(htmlElement, xAxis, yAxis, xData, yData, lineColor, dotColor, dummyName = '') {

    let combinedData = xData.map((d, i) => ([d, yData[i]]));
  ...
  drawBars(htmlElement, xAxis, yAxis, xData, yData, height, dummyName = '') {

    const combinedData = xData.map((d, i) => ({ xData: d, yData: yData[i] }));
  ...
  drawInteractiveBars(htmlElement, xAxis, yAxis, xData, yData, height, marginT, dummyName = '') {

    const combinedData = xData.map((d, i) => ({ xData: d, yData: yData[i] }));
    const normalColor = this.cssObject.barColorNormal;
    ...
    return htmlElement.selectAll(dummyName)
      ...
      .on('mouseenter', function(actual, i) {
        ...
        d3.select(this)
          .transition()
          .duration(300)
          .attr('x', (d: { xData: number }) => xAxis(d.xData) - 5)
      ...
      .on('mouseleave', function() {
        ...
        d3.select(this)
          .transition()
          .duration(300)
          .attr('x', (d: { xData: number }) => xAxis(d.xData))
          .attr('width', xAxis.bandwidth())
          .style('fill', normalColor);
      });
  }

  drawBarChartText(htmlElement, xAxis, yAxis, xData, yData, height, marginT, dummyName = '') {
  ...
  drawMinMaxMean(htmlElement, dataArray: ArrayLike<number>, x1, x2, yStart, fontSize, unit = '') {
  ...
  drawLineChartNoDataDashArray(htmlElement, xAxis, yAxis, xData, yData, lineColor, dotColor, hollowIndex, dummyName = '') {

    let lineChart = this.drawLineChart(htmlElement, xAxis, yAxis,
      xData, yData, lineColor, dotColor, dummyName);

    lineChart[2]
      .filter((d, i) => hollowIndex[i])
      .style('fill', 'transparent')
