...
@Component({
  selector: 'page-databarchart',
  templateUrl: 'databarchart.html',
})

export class DatabarchartPage {
  width: number;
  height: number;
    ...
    **/
    ...
    if (this.dataType == "time-point" && validData.length > 0) {
      if (this.className == "Woke-up Time"){
        ...
        let WokeupTimestampMean = Math.round(d3Array.mean(validData, (d) => d + new Date().setHours(4, 0, 0, 0)));
        minMaxMean[1].text(`${this.datetimeService.getDigitalClockTime(WokeupTimestampMean)}`);
        let WokeupTimestampMax = Math.round(d3Array.max(validData, (d) => d + new Date().setHours(4, 0, 0, 0)));
        minMaxMean[3].text(`${this.datetimeService.getDigitalClockTime(WokeupTimestampMax)}`);
        let WokeupTimestampMin = Math.round(d3Array.min(validData, (d) => d + new Date().setHours(4, 0, 0, 0)));
        ...
        let timestampMean = Math.round(d3Array.mean(validData, (d) => d + new Date().setHours(0, 0, 0, 0)));
        minMaxMean[1].text(`${this.datetimeService.getDigitalClockTime(timestampMean)}`);
        let timestampMax = Math.round(d3Array.max(validData, (d) => d + new Date().setHours(0, 0, 0, 0)));
        minMaxMean[3].text(`${this.datetimeService.getDigitalClockTime(timestampMax)}`);
        let timestampMin = Math.round(d3Array.min(validData, (d) => d + new Date().setHours(0, 0, 0, 0)));
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad DatabarchartPage');
  }
